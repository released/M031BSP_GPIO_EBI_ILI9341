...
void setup_TFT_Float_Test(void) {
  // Serial.begin(115200);
  // init();
  setRotation(3);
...
void loop_TFT_Float_Test(void) {
 ...
 float test = 0.0f;
  ...
  test = 67.125f;
  drawFloat(test, 4, 160, 120, 6);
  sprintf(tmp,"%4.4f" , test);//dtostrf(test,4,4,tmp) 
    drawString(tmp, 100, 200, 6);

  delay(1000);
  fillScreen(TFT_BLACK);
  
  test = -0.555555f;
  drawFloat(test, 3, 160, 120, 6);
  sprintf(tmp,"%2.2f" , test);//dtostrf(test,2,2,tmp)
    drawString(tmp, 100, 200, 6);
