...
static int fillSegment(int x, int y, int start_angle, int sub_angle, int r, unsigned int colour);

void setup_TFT_Pie_Chart(void)
{
  // begin();
...
static int fillSegment(int x, int y, int start_angle, int sub_angle, int r, unsigned int colour)
{
  ...
  for (i = start_angle; i < start_angle + sub_angle; i++) {

    // Calculate pair of coordinates for segment end
    x2 = cos((i + 1 - 90) * DEG2RAD) * r + x;
    y2 = sin((i + 1 - 90) * DEG2RAD) * r + y;

    fillTriangle(x1, y1, x2, y2, x, y, colour);
