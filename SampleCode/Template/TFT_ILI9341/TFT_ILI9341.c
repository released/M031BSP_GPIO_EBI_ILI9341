...
extern unsigned int ADC_ReadChannel(uint8_t ch);

uint8_t g_InitOnce = 0;
...
// TFT_ILI9341(int16_t w, int16_t h)
void init_parameter(void)
{
  int16_t w = ILI9341_TFTWIDTH;
  int16_t h = ILI9341_TFTHEIGHT;

  #if 0
  if (TFT_RST > 0) {
    TFT_RST = LOW ; //digitalWrite(TFT_RST, LOW);
...
void commandList (const uint8_t *addr)
{
	...
	spi_begin();
	numCommands = *(__IO uint8_t *)(addr++);            // Number of commands to follow
	while (numCommands--)                           // For each command...
	{
		writecommand(*(__IO uint8_t *)(addr++));    // Read, issue command
		numArgs = *(__IO uint8_t *)(addr++);        // Number of args to follow
		ms = numArgs & ILI9341_INIT_DELAY;      // If hibit set, delay follows args
		numArgs &= ~ILI9341_INIT_DELAY;         // Mask out delay bit
		while (numArgs--)                       // For each argument...
		{
			writedata(*(__IO uint8_t *)(addr++)); // Read, issue argument
		}

		if (ms)
		{
			ms = *(__IO uint8_t *)(addr++);     // Read post-command delay time (ms)
			delay( (ms==255 ? 500 : ms) );
		}
	}
	spi_end();
...
void drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color) {
  ...
  for (j = 0; j < h; j++) {
    for (i = 0; i < w; i++ ) {
      if (*(__IO uint8_t *)(bitmap + j * byteWidth + i / 8) & (128 >> (i & 7))) {
...
int16_t textWidth(const char *string, int font)
{
  ...
  if (font>1 && font<9)
  widthtable = (char *)*(__IO uint16_t *)( &(fontdata[font].widthtbl ) ) - 32; //subtract the 32 outside the loop
  else return 0;
...
#endif
    str_width += *(__IO uint8_t *)( widthtable + uniCode); // Normally we need to subract 32 from uniCode
  }
...
{
  return *(__IO uint8_t *)( &fontdata[font].height ) * textsize;
}
...
***************************************************************************************/
void drawChar6(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size)
{
  boolean fillbg = 0;
  byte column[6];
  byte mask = 0x1;
...
// but it is not really worth the extra 168 bytes needed...
  if ((size==1) && fillbg)
  {
    ...
    for (i = 0; i < 5; i++ ) column[i] = *(__IO uint8_t *)(font + (c * 5) + i);
    column[5] = 0;
    ...
    for (j = 0; j < 8; j++) {
      for (k = 0; k < 5; k++ ) {
        if (column[k] & mask) {
          // while (!(SPSR & _BV(SPIF)));
          writedata( color >> 8); //asm volatile( "nop\n\t" ::); // Sync to SPIF bit
          // while (!(SPSR & _BV(SPIF)));
          writedata( color);
        }
        else {
          // while (!(SPSR & _BV(SPIF)));
          writedata( bg >> 8); //asm volatile( "nop\n\t" ::);
          // while (!(SPSR & _BV(SPIF)));
          writedata( bg);
        }
      }
      ...
      mask <<= 1;
      // while (!(SPSR & _BV(SPIF)));
      writedata( bg >> 8); //while (!(SPSR & _BV(SPIF)));
      writedata( bg);
    }
    // while (!(SPSR & _BV(SPIF)));
    TFT_CS_H;
...
void setAddrWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
{
  ...
  TFT_DC_D;
  writedata( x0 >> 8); spiWait12();
  addr_col = 0xFFFF;
  writedata( x0); spiWait12();
  if(x1!=win_xe) {
    writedata( x1 >> 8); spiWait12();
    //asm volatile( "nop\n\t" ::);
    win_xe=x1;
    writedata( x1); spiWait14();
  }

  // Row addr set
  TFT_DC_C;
  writecommand( ILI9341_PASET); spiWait15();

  TFT_DC_D;
  writedata( y0 >> 8); spiWait12();
  addr_row = 0xFFFF;
  writedata( y0); spiWait12();
  if(y1!=win_ye) {
    writedata( y1 >> 8); spiWait12();
    //asm volatile( "nop\n\t" ::);
    win_ye=y1;
    writedata( y1); spiWait14();
  }

  // write to RAM
  TFT_DC_C;
  writecommand( ILI9341_RAMWR); spiWait14();
...
void setAddrWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1)
{
  ...
  TFT_DC_D;
  spiwrite( x0 >> 8; spiWait17();
  spiwrite( x0; spiWait17();
  spiwrite( x1 >> 8; spiWait17();
  spiwrite( x1; spiWait14();

  // Row addr set
  TFT_DC_C;
  spiwrite( ILI9341_PASET; spiWait15();

  TFT_DC_D;
  spiwrite( y0 >> 8; spiWait17();
  spiwrite( y0; spiWait17();
  spiwrite( y1 >> 8; spiWait17();
  spiwrite( y1; spiWait14();

  // write to RAM
  TFT_DC_C;
  spiwrite( ILI9341_RAMWR; spiWait14();
...
if (addr_col != x) {
  ...
  addr_col = x;
  TFT_DC_D;
  writedata( x >> 8); spiWait17();
  writedata( x); spiWait17();

  writedata( x >> 8); spiWait17();
  writedata( x); spiWait12();
}
...
if (addr_row != y) {
  ...
  addr_row = y;
  TFT_DC_D;
  writedata( y >> 8); spiWait17();
  writedata( y); spiWait17();

  writedata( y >> 8); spiWait17();
  writedata( y); spiWait14();
}

  TFT_DC_C;

  writecommand( ILI9341_RAMWR); spiWait15();

  TFT_DC_D;

  writedata( color >> 8); spiWait15();
  win_xe=x;
  writedata( color); spiWait12();
  win_ye=y;
...
if (addr_row != y) {
  ...
  writedata( color >> 8); spiWait17();
  writedata( color); spiWait14();
...
void fastSetup(void)
{
  ...
  spiWait15();
  TFT_DC_D;
  writedata( 0); spiWait14();
  addr_col = 0;
  writedata( 0); spiWait12();
  win_xe=_width-1;
  writedata( win_xe >> 8); spiWait15();
  writedata( win_xe); spiWait14();
  ...
  writedata( 0); spiWait12();
  win_ye=_height-1;
  writedata( win_ye >> 8); spiWait15();
  writedata( win_ye); spiWait14();
...
void drawPixel(uint16_t x, uint16_t y, uint16_t color)
{
  ...
  TFT_DC_D;
  spiwrite( x >> 8; spiWait17();
  spiwrite( x; spiWait15(); x++; 
  spiwrite( x >> 8; spiWait17();
  spiwrite( x; spiWait14();
  ...
  TFT_DC_D;
  spiwrite( y >> 8; spiWait17();
  spiwrite( y; spiWait15(); y++; 
  spiwrite( y >> 8; spiWait17();
  spiwrite( y; spiWait14();

  // write to RAM
  TFT_DC_C;
  spiwrite( ILI9341_RAMWR; spiWait15();

  TFT_DC_D;

  spiwrite( color >> 8; spiWait17();
  spiwrite( color; spiWait14();
...
{
  int16_t dx = 0, dy = 0;
  int16_t err = 0, ystep = 0, xs = 0, dlen = 0;
...
***************************************************************************************/
uint8_t write(uint8_t uniCode) //size_t write(uint8_t uniCode)
{
  ...
  if (textfont == 2)
  {
      ...
      width = *(__IO uint8_t *)(widtbl_f16 + uniCode-32);
      height = chr_hgt_f16;
...
#ifdef LOAD_RLE
  {
      ...
      width = *(__IO uint8_t *)( *(__IO uint16_t *)( &(fontdata[textfont].widthtbl ) ) + uniCode-32 );
      height= *(__IO uint8_t *)( &fontdata[textfont].height );
  }
#endif
  ...
  else
  {
    ...
    cursor_x += drawChar4(uniCode, cursor_x, cursor_y, textfont);
  }
  return 1;
}
...
int drawChar4(unsigned int uniCode, int x, int y, int font)
{
  ...
  {
#ifdef LOAD_GLCD
      drawChar6(x, y, uniCode, textcolor, textbgcolor, textsize);
      return 6 * textsize;
#else
      return 0;
#endif
  ...
  if (font == 2)
  {
      ...
      flash_address = *(__IO uint16_t *)(&chrtbl_f16[uniCode]);
      width = *(__IO uint8_t *)(widtbl_f16 + uniCode);
      height = chr_hgt_f16;
  }
  #ifdef LOAD_RLE
  else
...
#ifdef LOAD_RLE
  {
      ...
      flash_address = *(__IO uint16_t *)( *(__IO uint16_t *)( &(fontdata[font].chartbl ) ) + uniCode*sizeof(void *) );
      width = *(__IO uint8_t *)( *(__IO uint16_t *)( &(fontdata[font].widthtbl ) ) + uniCode );
      height= *(__IO uint8_t *)( &fontdata[font].height );
  }
#endif
  ...
  if (font == 2) {
    ...
    if (textcolor == textbgcolor || textsize != 1) {
      ...
      for (i = 0; i < height; i++)
      {
        ...
        for (k = 0; k < w; k++)
        {
          line = *(__IO uint8_t *)(flash_address + w * i + k);
          if (line) {
            if (textsize == 1) {
              pX = x + k * 8;
      ...
      for (i = 0; i < height; i++)
      {
        for (k = 0; k < w; k++)
        {
          ...
          while (mask) {
            if (line & mask) {
              // while (!(SPSR & _BV(SPIF)));
              writedata( th); //asm volatile( "nop\n\t" ::);
              // while (!(SPSR & _BV(SPIF)));
              writedata( tl);
            }
            else {
              // while (!(SPSR & _BV(SPIF)));
              writedata( bh); //asm volatile( "nop\n\t" ::);
              // while (!(SPSR & _BV(SPIF)));
              writedata( bl);
            }
            mask = mask >> 1;
  ...
  {
    spi_begin();
    // spiwrite( 0); // Dummy write to ensure SPIF flag gets set for first check in while() loop
    ...
    if ((textsize != 1) || (textcolor == textbgcolor)) {
      ...
      pc = 0; // Pixel count
      np = textsize * textsize; // Number of pixels in a drawn pixel

      tnp = 0; // Temporary copy of np for while loop
      ts = textsize - 1; // Temporary copy of textsize
      // 16 bit pixel count so maximum font size is equivalent to 180x180 pixels in area
      // w is total number of pixels to plot to fill character block
      while (pc < w)
      {
        line = *(__IO uint8_t *)(flash_address);
         ...
         // so use faster drawing of characters and background using block write
    {
      ...
      while (w > 0)
      {
        line = *(__IO uint8_t *)(flash_address++); // 8 bytes smaller when incrementing here
        if (line & 0x80) {
          line &= 0x7F;
          line++; w -= line;
...
int drawString(const char *string, int poX, int poY, int font)
{
  ...
  if (textdatum || padX)
  {
    ...
    cheight = *(__IO uint8_t *)( &fontdata[font].height ) * textsize;
  ...
  while (*string) sumX += drawChar4(*(string++), poX+sumX, poY, font);
...
int drawFloat(float floatNumber, int dp, int poX, int poY, int font)
{
  ...
  // Get integer part
  temp = (unsigned long)floatNumber;
